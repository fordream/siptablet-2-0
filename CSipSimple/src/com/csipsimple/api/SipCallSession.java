...
public class SipCallSession implements Parcelable {
	 ...
	 * The value must be a bundle with key representing header name, and value
	 * representing header value.
	 * 
	 * @see Bundle
	 */
	...
	/**
	 * The call signaling is secure until it arrives on server. After, nothing
	 * ensures how it goes.
	 */
	...
	/**
	 * Check if the call state indicates that it is an active call in progress.
	 * This is equivalent to state incoming or early or calling or confirmed or
	 * connecting
	 * 
	 * @return true if the call can be considered as in progress/active
	 */
	public boolean isActive() {
		return (callState == InvState.INCOMING || callState == InvState.EARLY || callState == InvState.CALLING || callState == InvState.CONFIRMED || callState == InvState.CONNECTING);
	}

	/**
	 * Chef if the call state indicates that it's an ongoing call. This is
	 * equivalent to state confirmed.
	 * 
	 ...
	 * 
	 * @return one of {@link #TRANSPORT_SECURE_NONE},
	 *         {@link #TRANSPORT_SECURE_TO_SERVER},
	 *         {@link #TRANSPORT_SECURE_FULL}
	 */
	...
	/**
	 * Check if the specific call info indicates that it is a call that has not
	 * yet been confirmed by both ends.<br/>
	 * In other worlds if the call is in state, calling, incoming early or
	 * connecting.
	 * 
	 * @return true if the call can be considered not yet been confirmed
	 */
	public boolean isBeforeConfirmed() {
		return (callState == InvState.CALLING || callState == InvState.INCOMING || callState == InvState.EARLY || callState == InvState.CONNECTING);
	}

	/**
	 * Check if the specific call info indicates that it is a call that has been
	 * ended<br/>
	...
	protected String webSipId = null;

	public String getWebSipId() {
		return webSipId;
	}
