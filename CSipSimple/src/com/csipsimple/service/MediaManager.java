...
public class MediaManager implements BluetoothChangeListener {
	...
	private synchronized void actualSetAudioInCall() {
		...
		if (wifiLock == null) {
			wifiLock = wman.createWifiLock((Compatibility.isCompatible(9)) ? WifiManager.WIFI_MODE_FULL_HIGH_PERF : WifiManager.WIFI_MODE_FULL, "com.csipsimple.InCallLock");
		...
		setStreamVolume(inCallStream, (int) (audioManager.getStreamMaxVolume(inCallStream) * service.getPrefs().getInitialVolumeLevel()), 0);
	...
	/**
	 * Start ringing announce for a given contact. It will also focus audio for
	 * us.
	 * 
	 * @param remoteContact
	 *            the contact to ring for. May resolve the contact ringtone if
	 *            any.
	 */
	...
	/**
	 * Helper class to play tones through the earpiece (or speaker / BT) during
	 * a call, using the ToneGenerator.
	 * 
	 * To use, just instantiate a new InCallTonePlayer (passing in the TONE_*
	 * constant for the tone you want) and start() it.
	 * 
	 * When we're done playing the tone, if the phone is idle at that point,
	 * we'll reset the audio routing and speaker state. (That means that for
	 * tones that get played *after* a call disconnects, like "busy" or
	 * "congestion" or "call ended", you should NOT call
	 * resetAudioStateAfterDisconnect() yourself. Instead, just start the
	 * InCallTonePlayer, which will automatically defer the
	 * resetAudioStateAfterDisconnect() call until the tone finishes playing.)
	 */
